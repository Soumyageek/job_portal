...
class Linkedin:
    ...
    def __init__(self, linkdin_url):
        ...
        # self.driver = webdriver.Chrome(f"{path}/chromedriver", chrome_options=options)
        actions.login(self.driver, email, password)
        self.driver.get(linkdin_url)
    ...
    def get_score(self, employee_skills):
        ...
        for section in range(1, total_section + 1):
            ...
            for tools_and_tech in range(1, total_tools_and_tech + 1):
                sub_skill_prefix_xpath = f"{sub_skills_prefix_xpath}[{tools_and_tech}]"

                tools_and_tech_text = self.driver.find_element_by_xpath(f"{sub_skill_prefix_xpath}//span[@class='pv-skill-category-entity__name-text t-16 t-black t-bold']").text
...
# print(lnk.get_score([]))
#
# l =Linkedin("https://www.linkedin.com/in/abhishek-jaiswal-02742212b/")
# l.get_score("python,java")